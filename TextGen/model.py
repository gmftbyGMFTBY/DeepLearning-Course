...
class CaptionGenerator(BaseModel):
    ...
    def build_vgg16(self):
        ...
        # conv1_1_feats = self.nn.conv2d(images, 8, name='conv1_1')
        # conv1_2_feats = self.nn.conv2d(conv1_1_feats, 8, name='conv1_2')
        # pool1_feats = self.nn.max_pool2d(conv1_2_feats, name='pool1')
        #
        # conv2_1_feats = self.nn.conv2d(pool1_feats, 16, name='conv2_1')
        # conv2_2_feats = self.nn.conv2d(conv2_1_feats, 16, name='conv2_2')
        # pool2_feats = self.nn.max_pool2d(conv2_2_feats, name='pool2')
        #
        # conv3_1_feats = self.nn.conv2d(pool2_feats, 32, name='conv3_1')
        # conv3_2_feats = self.nn.conv2d(conv3_1_feats, 32, name='conv3_2')
        # conv3_3_feats = self.nn.conv2d(conv3_2_feats, 32, name='conv3_3')
        # pool3_feats = self.nn.max_pool2d(conv3_3_feats, name='pool3')
        #
        # conv4_1_feats = self.nn.conv2d(pool3_feats, 64, name='conv4_1')
        # conv4_2_feats = self.nn.conv2d(conv4_1_feats, 64, name='conv4_2')
        # conv4_3_feats = self.nn.conv2d(conv4_2_feats, 64, name='conv4_3')
        # pool4_feats = self.nn.max_pool2d(conv4_3_feats, name='pool4')
        #
        # conv5_1_feats = self.nn.conv2d(pool4_feats, 64, name='conv5_1')
        # conv5_2_feats = self.nn.conv2d(conv5_1_feats, 64, name='conv5_2')
        # conv5_3_feats = self.nn.conv2d(conv5_2_feats, 64, name='conv5_3')

        reshaped_conv5_3_feats = tf.reshape(conv5_3_feats,
                                            [config.batch_size, 196, 512])

        # reshaped_conv5_3_feats = tf.reshape(conv5_3_feats,
        #                                             [config.batch_size, 196, 64])
